...
def is_emserv(row, description):
    dep = Department.query.filter(
        Department.name == "Emergency Services").first()
...
def is_medex(row, description):
    dep = Department.query.filter(
        Department.name == "Medical Examiner's").first()
...
def is_econ(row, description):
    dep = Department.query.filter(
        Department.name == 'Economic Development').first()
...
def categorize(credit):
    tests = [is_kane, is_health, is_emserv, is_medex, is_sheriff, is_police, is_jail,
             is_econ, is_tax, is_drink, is_child, is_parental, is_blood, is_parks, is_cage]
...
def add_to_db(row, dep_id):
    credit = ACH(amount=row["Amount"], fund=row["AccountName"][-5:],
                 description=row["Description"], received=row["AsOfDate"], department_id=dep_id)
    db.session.add(credit)
    db.session.commit()


def process_ach(ach_file):
    reader = csv.DictReader(ach_file)
    for row in reader:
        if is_credit(row):
            categorize(row)
